...
async function execute(interaction) {
  ...
  if (interaction.user.id === deal.buyerId) {
    await completeDeal(dealId, interaction.user.id, 'buyer');
  } else if (interaction.user.id === deal.sellerId) {
    await completeDeal(dealId, interaction.user.id, 'seller');
  } else {
    return interaction.reply({ content: 'You are not part of this deal.', ephemeral: true });
  }

  return interaction.reply({ content: `Final confirmation recorded for ${dealId}.`, ephemeral: true });
}
